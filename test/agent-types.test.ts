...
describe("agent type registry", () => {
  ...
  describe("default agents", () => {
    ...
    it("Explore has read-only tools plus FFF search extensions", () => {
      ...
      expect(config.description).toContain("context-building");
    ...
    it("Systematic Debugging is a read-only root-cause investigator", () => {
      ...
      expect(config.builtinToolNames).not.toContain("write");
      expect(config.extensions).toEqual(["ffgrep", "fffind", "fff-multi-grep"]);
      expect(agentConfig?.thinking).toBe("high");
      expect(agentConfig?.maxTurns).toBe(24);
      expect(agentConfig?.maxTokens).toBe(350_000);
    });
