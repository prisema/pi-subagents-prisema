...
| `Implement` | read, bash, edit, write, grep, find, ls + FFF search tools | inherit | `replace` (standalone) | Superpowers-style TDD implementation agent |
| `Review` | read, bash, grep, find, ls + FFF search tools | inherit | `replace` (standalone) | Evidence-driven reviewer/validator (read-only) |
| `Remove Slop` | read, bash, edit, grep, find, ls + FFF search tools | inherit | `replace` (standalone) | Scoped cleanup for AI-generated slop after implementation/validation |

The `general-purpose` agent is a **parent twin** — it receives the parent's entire system prompt plus a sub-agent context bridge, so it follows the same rules the parent does. Explore uses a standalone read-only prompt for concise local Context Packs before implementation. Web Research uses the Pi web access tools (`web_search`, `fetch_content`, `get_search_content`, `code_search`) to return filtered, source-backed Web Context Packs for external/current research. Systematic Debugging uses a standalone read-only root-cause workflow for bugs, test failures, build failures, and unexpected behavior before any fix is proposed. SEO GEO Agent Search is the Prisema marketing specialist for SEO/GEO, AI Search, Agent Search, llms.txt, schema, robots, and AI-ready public discovery work; the legacy custom type spelling `seo_geo_agent_search` resolves to it. Plan uses a controlled-write standalone prompt that may write only planning artifacts under `.pi/taskdone/plans/<plan-id>/` while producing an approval-gated Taskdone Planning Package with `plan.md`, `taskdone.json`, a detailed task catalog in both artifacts, and a visible task preview in its final response. For ambiguous creative work, Plan applies the Prisema Brainstorming Gate: it gathers context, asks only blocking questions, presents 2-3 approaches, and records the approved direction before any Implement task runs. Implement is the scoped TDD code-writing role. Review is the read-only evidence gate for acceptance criteria and fix requests. Remove Slop is the scoped post-validation cleanup role for touched files. Most specialized local-code agents allowlist FFF search extension tools (`fffind`, `ffgrep`, `fff-multi-grep`) when `@ff-labs/pi-fff` is installed. SEO GEO Agent Search intentionally inherits all extension tools because marketing audits may need web, browser, MCP, or media capabilities.
...
`Explore` and `Web Research` have embedded 36-turn caps so broad mapping/research jobs return incomplete-but-usable Context Packs instead of continuing expensive searches. Override by ejecting/customizing either agent if a project needs a different budget.
